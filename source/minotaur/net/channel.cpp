...
int Channel::Start() {
  ...
  if (0 != SocketOperation::SetNoDelay(GetFD())) {
    MI_LOG_ERROR(logger, "Channel::TryConnect SetNoDelay, failed with:"
...
int Channel::DecodeMessage() {
  ...
  while (result == Protocol::kDecodeSuccess && read_buffer_.GetReadSize()) {
    ProtocolMessage* message = GetProtocol()->Decode(&read_buffer_, &result, NULL);
    if (!message) {
      continue;
    }
